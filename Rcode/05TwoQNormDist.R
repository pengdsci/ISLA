...
ui <- fluidPage(
  ...
  tags$div(HTML("<script type='text/x-mathjax-config' >
            ...
            ")),
  br(),
  ### Important: The following tag allows inline equation in MathJax  #acdfe6; #005960
  wellPanel(style = "background-color: #5e2442",
            titlePanel( 
              h3("ISLA: Two Types of Questions of Normal Distributions", 
    ...
    fluidRow(
     ...
     ###########################################################################   
        #column(4, wellPanel( style = "background-color:lightgray", 
            #####
...
server <- function(input, output){
    ...
    output$genNormCurve <- renderPlot({
        ...
        if(input$problem_type=='1' & input$range_type == '1'){ # between
            ...
            if(V0 < (mu-4*s.xbar) & V1 < (mu+4*s.xbar) ){
                ...
                text(c((mu-4*s.xbar),V1), c(-0.08*ymax,-0.08*ymax), c(expression('V'[0]), expression('V'[1])), col = "red", cex = 1.5)    
                ...
                text(c((mu -4*s.xbar),(mu+4*s.xbar)), c(-0.08*ymax,-0.08*ymax), c(expression('V'[0]), expression('V'[1])), col = "red", cex = 1.5)    
            } else{
                v.seq = seq(V0, V1, length = 200)
                y.seq = dnorm(v.seq, mu, s.xbar)
                polygon(c(V0,v.seq, V1),c(0,y.seq,0),col="lightblue")
                text(c(V0,V1), c(-0.08*ymax,-0.08*ymax), c(expression('V'[0]), expression('V'[1])), col = "red", cex = 1.5)
            }
            #####
            text(mu-2.5*s.xbar, 0.92*ymax, "Given Information", col = "red", cex = 1.5)
            text(mu-2.5*s.xbar, 0.8*ymax, paste('V0 = ',V0), col = "red", cex =1.5)
            text(mu-2.5*s.xbar, 0.69*ymax, paste('V1 = ',V1), col = "red", cex = 1.5)
        }
        ######
        if(input$problem_type=='1' & input$range_type == '2'){ # between
            ...
            if(GV < (mu+4*s.xbar) & GV > (mu -4*s.xbar)){
               ...
               text(c(GV), c(-0.08*ymax), c(expression('V'[0])), col = "red", cex = 1.5)
            } else if (GV < (mu-4*s.xbar) ){
                v.seq = seq((mu-4*s.xbar), mu+4*s.xbar, length = 200)
                y.seq = dnorm(v.seq, mu, s.xbar)
                polygon(c((mu-4*s.xbar),v.seq, mu+4*s.xbar),c(0,y.seq,0),col="lightblue")
                text(c((mu-4*s.xbar)), c(-0.08*ymax), c(expression('V'[0])), col = "red", cex = 1.5)
                text(c(mu-4*s.xbar), c(-0.08*ymax), c(expression('V'[0])), col = "red", cex = 1.5)                
            } else{
                text(c(mu+4*s.xbar), c(-0.08*ymax), c(expression('V'[0])), col = "red", cex = 1.5)  
            }
            ####
              text(mu-2.5*s.xbar, 0.92*ymax, "Given Information", col = "red", cex = 1.5)
              text(mu-2.5*s.xbar, 0.72*ymax, paste('V0 = ',GV), col = "red", cex = 1.5)
         }
        ######
        if(input$problem_type=='1' & input$range_type == '3'){ # between
            ...
            if(LV > mu-4*s.xbar & LV < mu + 4*s.xbar){
               ...
               text(c(LV), c(-0.08*ymax), c(expression('V'[0])), col = "red", cex = 1.5)
            ...
            ###            
            text(mu-2.5*s.xbar, 0.92*ymax, "Given Information", col = "red", cex = 1.5)
            text(mu-2.5*s.xbar, 0.72*ymax, paste('V0 = ',LV), col = "red", cex = 1.5)
        }
        ############################
        ######  Quantile  ##########
        ############################
        ...
        if(input$problem_type=='2' & input$prob_type == '1'){
         ...
         if(V1 < (mu + 4*s.xbar) ){
             ...
             y.seq = dnorm(v.seq, mu, s.xbar)
             polygon(c(V0,v.seq, V1),c(0,y.seq,0),col="lightblue") 
             text(c(V0,V1), c(-0.08*ymax,-0.08*ymax), c(expression('X'[0]), expression('V')), col = "red", cex = 1.5)
         } else {
             v.seq = seq(V0,(mu +4*s.xbar), length = 200)
             y.seq = dnorm(v.seq, mu, s.xbar)
             polygon(c(V0,v.seq, (mu + 4*s.xbar)),c(0,y.seq,0),col="lightblue") 
             text(c(V0,(mu +4*s.xbar)), c(-0.08*ymax,-0.08*ymax), c(expression('X'[0]), expression('V')), col = "red", cex = 1.5)    
         } 
         ###
         text(mu-2.5*s.xbar, 0.92*ymax, "Given Information", col = "red", cex = 1.5)
         text(mu-2.5*s.xbar, 0.8*ymax, paste('P0 = ',p0), col = "red", cex = 1.5)
         text(mu-2.5*s.xbar, 0.68*ymax, paste('V = ',rV), col = "red", cex = 1.5)
         }
         ##### P[V < bar(X) < Xo]
        if(input$problem_type=='2' & input$prob_type == '2'){
            ...
            if(lV < (mu - 4*s.xbar)){
                ...
                y.seq = dnorm(v.seq, mu, s.xbar)
                polygon(c((mu - 4*s.xbar),v.seq, V1),c(0,y.seq,0),col="lightblue") 
                text(c((mu - 4*s.xbar),V1), c(-0.08*ymax,-0.08*ymax), c(expression('V'), expression('X'[0])), col = "red", cex = 1.5) 
            } else {
               v.seq = seq(V0, V1, length = 200)
               y.seq = dnorm(v.seq, mu, s.xbar)
               polygon(c(V0,v.seq, V1),c(0,y.seq,0),col="lightblue") 
               text(c(V0,V1), c(-0.08*ymax,-0.08*ymax), c(expression('V'), expression('X'[0])), col = "red", cex = 1.5)
            }
            ##
            text(mu-2.5*s.xbar, 0.92*ymax, "Given Information", col = "red", cex = 1.5)
            text(mu-2.5*s.xbar, 0.8*ymax, paste('P0 = ',p0), col = "red", cex = 1.5)
            text(mu-2.5*s.xbar, 0.68*ymax, paste('V = ',lV), col = "red", cex = 1.5)
        }
        ##### P[ bar(X) > Xo]
        if(input$problem_type=='2' & input$prob_type == '3'){
            ...
            text(c(rQ), c(-0.08*ymax), c(expression('X'[0])), col = "red", cex = 1.5)
            ###
            text(mu-2.5*s.xbar, 0.92*ymax, "Given Information", col = "red", cex = 1.5)
            text(mu-2.5*s.xbar, 0.8*ymax, paste('P0 = ',p0), col = "red", cex = 1.5)
        }
        ...
        if(input$problem_type=='2' & input$prob_type == '4'){
            ...
            text(c(lQ), c(-0.08*ymax), c(expression('X'[0])), col = "red", cex = 1.5)
            ###
            text(mu-2.5*s.xbar, 0.92*ymax, "Given Information", col = "red", cex = 1.5)
            text(mu-2.5*s.xbar, 0.8*ymax, paste('P0 = ',p0), col = "red", cex = 1.5)
        }
        ...
        text(1,1, expression(Z == frac(x-mu, sigma)), cex = 2, col = "darkred")
       ...
       # par(oma = c(1,1,0,1), mar = c(1,1,1,1), bg = "#f7f7f7")
        ...
        if(input$problem_type=='2' & input$prob_type == '1'){ # between
          ...
          if(Z1 > 4){
            ...
            polygon(c(-4,z.seq02, 4),c(0,yz.seq02,0), col= "lightblue" )
            segments(z.seq01, rep(0,round(8*(4+4))), z.seq01, dnorm(z.seq01), col= "darkred" )
            text(c(Z0,4), c(-0.08*zmax,-0.08*zmax), c(expression('Z'[0]), expression('Z'[V])), col = "blue", cex = 1.5)
          }
          if(Z1 < 4){
            z.seq01 = seq(-4, Z0, length = round(8*(Z0+4)))
            z.seq02 = seq(-4, Z1, length = 200)
            ...
            polygon(c(-4,z.seq02, Z1),c(0,yz.seq02,0), col= "lightblue" )
            segments(z.seq01, rep(0,round(8*(Z1+4))), z.seq01, dnorm(z.seq01), col= "darkred" )
            text(c(Z0,Z1), c(-0.08*zmax,-0.08*zmax), c(expression('Z'[0]), expression('Z'[V])), col = "blue", cex = 1.5)
          }
        ...
        if(input$problem_type=='2' & input$prob_type == '2'){ # between
          ...
          if (Z0 < -4){
            ...
            ##
            text(c(-4,Z1), c(-0.08*zmax,-0.08*zmax), c(expression('Z'[V]), expression('Z'[0])), col = "blue", cex = 1.5)                
          } else{
            z.seq01 = seq(-4, Z0, length = round(8*(Z0+4)))
            z.seq02 = seq(-4, Z1, length = 200)
            yz.seq01 = dnorm(z.seq01, 0, 1)
            ...
            segments(z.seq01, rep(0,round(8*(Z1+4))), z.seq01, dnorm(z.seq01), col= "darkred" )
            ##
            text(c(Z0,Z1), c(-0.08*zmax,-0.08*zmax), c(expression('Z'[V]), expression('Z'[0])), col = "blue", cex = 1.5)
          }
        }
        #####
        ###  P[ bar(X) > Xo]
        if(input$problem_type=='2' & input$prob_type == '3'){ # between
          ...
          ###
          zmax = dnorm(0)
          polygon(c(Z0,z.seq02, 4),c(0,yz.seq02,0), col= "lightblue" )
          ##
          text(c(Z0), c(-0.08*zmax), c(expression('Z'[0])), col = "blue", cex = 1.5)
        }
        #####
        ###  P[ bar(X) < Xo]
        if(input$problem_type=='2' & input$prob_type == '4'){ # between
          ...
          ###
          zmax = dnorm(0)
          polygon(c(-4,z.seq01, Z1),c(0,yz.seq01,0), col= "lightblue" )
          ##
          text(c(Z1), c(-0.08*zmax), c(expression('Z'[0])), col = "blue", cex = 1.5)
        }        
        ####    
    ###############################################################
   ...
   fluidPage(
      if(input$problem_type=='1' & input$range_type == '1'){ 
        ...
        #####################
          withMathJax(
                ...
                br(),
                br(),                  
                paste('Step 1. Recall Z-score Transformation'),
                #br(),
                paste0('$$ Z = \\frac{X -', mu, '}{',sigma, '} .$$'),
                #br(),
                paste('Step 2. Z-scores for $V_0=$', V0, 'and $V_1$=', V1, 'are given respectively by'),
                br(),
                paste0('$$Z_0 = \\frac{',V0, ' - ', mu, '}{',sigma, '} = ', Z0, '\\text{  and  }  Z_1 = \\frac{',V1, ' - ', mu, '}{',sigma, '} = ', Z1, '.$$'),
                paste('Step 3. The two left-tail Probabilities are'),
                paste0('$$P( Z <', Z1, ') = ',round(pnorm(Z1),4), '\\text{  and  } P( Z <', Z0, ') = ',round(pnorm(Z0),4), '.$$'),
                paste0('Step 4. Note that $$P \\left( Z_0 < Z < Z_1  \\right) = P \\left( Z < Z_1  \\right) - P \\left(Z < Z_0  \\right)$$'),
                paste0('$$= P \\left( Z < ',Z1,  '\\right) - P \\left(Z < ',Z0 , '\\right)$$'),
                paste0('$$= ',round(pnorm(Z1),4) , ' - ',round(pnorm(Z0),4) , ' = ',round(pnorm(Z1),4)-round(pnorm(Z0),4), '.$$'),
                paste0('Step 5. Therefore, $$ P(', V0, '< X <', V1,') = ', round(pnorm(Z1),4)-round(pnorm(Z0),4) ,' .$$')
                )
            },
        #####
        if(input$problem_type=='1' & input$range_type == '2'){ 
            ...
            #####    
                withMathJax(
                    ...
                    #br(),
                    paste0('$$ Z = \\frac{ X -', mu, '}{',sigma, '} .$$'),
                    paste('Step 2. Z-scores for $V$ =', GV, 'is given by'),
                    br(),
                    paste0('$$Z_0 = \\frac{',GV, ' - ', mu, '}{',sigma, '} = ', GZ, '.$$'),
                    paste('Step 3. The left-tail Probability based on the above z-score is'),
                    paste0('$$P( Z < ', GZ, ') = ',round(pnorm(GZ),4), '.$$'),
                    paste0('Step 4. Note that $$ P \\left(  Z > Z_1  \\right) = 1- P \\left(Z < ',GZ , '\\right)= 1 - ',round(pnorm(GZ),4) , ' = ', round(1-pnorm(GZ),4), '.$$'),
                    paste0('Step 5. Therefore, $$ P( X >', GV,') = ', round(1-pnorm(GZ),4) ,' .$$')
                )
        },    
      if(input$problem_type=='1' & input$range_type == '3'){ 
          #####################
          ...
          withMathJax(
              ...
              #br(),
              paste0('$$ Z = \\frac{ X -', mu, '}{',sigma, '} .$$'),
              paste('Step 2. Z-scores for $V$ =', LV, 'is given by'),
              br(),
              paste0('$$Z_0 = \\frac{',LV, ' - ', mu, '}{',sigma, '} = ', LZ, '.$$'),
              paste('Step 3. The left-tail probability based on the above z-score is'),
              paste0('$$P( Z <', LZ, ') = ',round(pnorm(LZ),4), '.$$'),
              paste0('Step 4. Note that $$ P \\left(  Z < Z_0  \\right) = P \\left(Z < ',LZ , '\\right) =' ,round(pnorm(LZ),4) , '.$$'),
              paste0('Step 5. Therefore, $$ P( X <', LV,') = ', round(pnorm(LZ),4) ,' .$$')
          )
      },
      #####
      ############################
      ...
      if(input$problem_type=='2' & input$prob_type == '1'){
          ...
          withMathJax(
              strong('Question:', style="color:darkred; font-weight:bold;"),
              paste('Given $P(X_0 < X <', rV, ') =', p0, '$, what is $X_0$? '),
              br(),
              br(),
              strong('Solution:', style="color:darkred; font-weight:bold;"),
              paste('The answer is given in the following steps.'),
              ...
              paste0('$$ Z = \\frac{ X -', mu, '}{',sigma, '} .$$'),
              #br(),
              paste('Step 2. The Z-score for the given $V$ =', rV, 'and $X_0$ are given by'),
              br(),
              paste0('$$Z_V = \\frac{',V1, ' - ', mu, '}{',sigma, '} = ', Z1, '\\text{  and  } Z_0 = \\frac{X_0 -', mu, '}{',sigma, '} .$$'),
              paste('Step 3. Note that'),
              paste0('$$P( Z_0 <  Z <', Z1, ') =  P( Z <', Z1,') - P(Z <Z_0 )= ', p0, '.$$'),
              ###
              paste0('That is, $$P( Z < Z_0 ) =  P(Z <',Z0, ') -', p0,' = ',round(pnorm(Z1) -p0,4) ,',$$'),
              ###
              paste('which gives $Z_0  = ',round(qnorm(pnorm(Z1)-p0),2), '.$'),
              br(),
              br(),
              paste0('Step 4. Therefore, $X_0$ satisfies the following equation $$ Z_0 = \\frac{ X_0 -', mu, '}{',sigma, '}=', round(qnorm(pnorm(Z1)-p0),2), '.$$'),
              #paste0('Therefore, $$\\frac{X_0 -',mu, '}{',sigma, '} = Z_0 =', round(qnorm(pnorm(Z1)-p0),2), '.$$'), 
              paste0('Step 5. Solving for $X_0$, we have, $X_0 =',mu,'+ (', round(qnorm(pnorm(Z1)-p0),2),')\\times',sigma,' =', round(mu+round(qnorm(pnorm(Z1)-p0),2)*s.xbar,2),'.$')
          )
      },
      #####
      if(input$problem_type=='2' & input$prob_type == '2'){
          ...
          withMathJax(
              strong('Question:', style="color:darkred; font-weight:bold;"),
              paste('Given $P(',lV,'< X < X_0) =', p0, '$, what is $X_0$? '),
              br(),
              br(),
              strong('Solution:', style="color:darkred; font-weight:bold;"),
              paste('The answer is given in the following steps.'),
              ...
              paste0('$$ Z = \\frac{ X -', mu, '}{',sigma, '} .$$'),
              #br(),
              paste('Step 2. Find the Z-score for the given $V$ =', lV, 'and $X_0$ are given by'),
              br(),
              paste0('$$Z_V = \\frac{',V0, ' - ', mu, '}{',sigma, '} = ', Z0, '\\text{  and  } Z_0 = \\frac{X_0 - ', mu, '}{',sigma, '}.$$'),
              paste('Step 3. Note that the probaility equation in the question is equivalent to'),
              paste0('$$P(', Z0,' <  Z < Z_0 ) = ', p0, '\\Rightarrow  P(Z< Z_0)- P( Z< ',Z0, ') = ', p0,  '.$$'),
              paste0(' $$\\Longrightarrow P( Z < Z_0 ) = P(Z <',Z0, ') +', p0, ' = ',round(pnorm(Z0) +p0,4) ,',$$'),
              paste('  \  \  \     which gives $Z_0  = ',round(qnorm(pnorm(Z0)+p0),2), '.$'),
              br(),
              br(),
              paste0('Step 4. Therefore, $$\\frac{X_0 -',mu, '}{',sigma, '} = Z_0 =', round(qnorm(pnorm(Z0)+p0),2), '.$$'), 
              paste0('Step 5. Solving for $X_0$, we have  $$X_0 =',mu,'+ (', round(qnorm(pnorm(Z0)+p0),2),')\\times',sigma,' =', round(mu+round(qnorm(pnorm(Z0)+p0),2)*s.xbar,2),'.$$')
          )
      },        
      #####
      if(input$problem_type=='2' & input$prob_type == '3'){
          ...
          withMathJax(
              strong('Question:', style="color:darkred; font-weight:bold;"),
              paste('Given $P(X > X_0) =', p0, '$, what is $X_0$? '),
              br(),
              br(),
              strong('Solution:', style="color:darkred; font-weight:bold;"),
              paste('The answer is given in the following steps.'),
              ...
              paste0('$$ Z = \\frac{ X -', mu, '}{',sigma, '}. $$'),
              #br(),
              paste('Step 2. The Z-score corresponding to $X_0$ is given by'),
              paste0('$$ Z_0 = \\frac{ X_0 -', mu, '}{',sigma, '}. $$'),
              paste0('Note that $$P( Z  >  Z_0) = ', p0, ' \\text{  or equivalently  } P( Z  <  Z_0) = ', 1-p0, ',$$'),
              br(),

              paste0(' which gives,   $Z_0 = ', round(qnorm(1-p0),2), '.$'),
              br(),
              br(),
              paste0('Step 3. By the definition of the Z-score of $X_0$, we have  $$\\frac{X_0 -',mu, '}{',sigma, '} = Z_0 =', round(qnorm(1-p0),2), '.$$'), 
              paste0('Step 4. Therefore, $X_0 =',mu,'+ (', round(qnorm(1-p0),2),')\\times',sigma,' = ', round(mu+round(qnorm(1-p0),2)*s.xbar,2),'.$'),
              br()
          )
      }, 
      #####
      if(input$problem_type=='2' & input$prob_type == '4'){
          ...
          withMathJax(
              ...
              br(),
              br(),
              paste('Step 1. Recall Z-score Transformations'),
              #br(),
              paste0('$$ Z = \\frac{ X -', mu, '}{',sigma, '} \\text{  and } Z = \\frac{ X_0 -', mu, '}{',sigma, '}. $$'),
              #br(),
              paste('Step 2. The given condition $P(X < X_0) =', p0, '$ is eqivalent to'),
              paste0('$$P( Z  <  Z_0) = ', p0, '$$'),
              br(),
              paste(' which gives,   $Z_0 = ', round(qnorm(p0),2), '.$'),
              br(),
              br(),
              paste0('Step 3. Note that, $$\\frac{X_0 -',mu, '}{',sigma, '} = Z_0 =', round(qnorm(p0),2), '.$$'), 
              paste0('Step 4. Solve for $X_0$ from the above equation, we have, $$X_0 =',mu,'+ (', round(qnorm(p0),2),')\\times',sigma,' =', round(mu+round(qnorm(p0),2)*s.xbar,2),'.$$'),
              br()
          )
      },      
    ) # close fluidPage  
